...
###mi manca questo: **exp(beta) and odds for the prediction of success**

We will now present some different approaches for the classification of the data.

## 3.1 Logistic Regression

Logistic regression can be defined as a type of generalized linear model (GLM). In particular, it is a statistical model used to examine the association of categorical or continuous independent variables with one dichotomous dependent variable. Therefore we start with it because we are dealing with "1" in case of stroke and "0" in case of no-stroke.  
...
In this part of the predictive analysis we will present three different type of models, compared to discover the best one that can better interpret the data. We will apply the Akaike Information Criterion (AIC) as selection of the candidate models (value given directly from the function "summary"). We decide to exclude the Mallow's Cp and the R^2 adjusted techniques because they work efficiently in the case of linear regression.

### 3.1.1 Full and Reduced Models

We begin with the full model to see how all the dataset features contribute on the prediction of a stroke.
```{r, fig.dim=c(3.5,3.5)}
...
We now make some tests in order to find the best reduced model: we take the full model and we remove all the predictors that show up collinearity between each other, i.e. `work_type`, `Residence_type` and `ever_married`, following a sort of greedy backward selection:
```{r}
...
Also in this case the variables more important are the same of the ones found in the full model, with the difference of `heart_disease`  that seems to bring about an improvement on the prediction.
...
The variables left to eliminate are `bmi` and `smoking_status`.   
Now we can define the final reduced model:
...
summary(mod.red)
```
For this model, we give a descriptive statistic through the residual plots:
```{r}
par(mfrow=c(2,2))
plot(mod.red)
par(mfrow=c(1,1))
...
**Scale-Location**  
We can see that homoscedasticity does not hold since the line of the standard residual is not flat, hence even by standardizing the residual we finish having high variance among residuals. But in our case, as we can notice from this plot, the red line is slightly curved and the residuals seem to increase as the fitted Y values increase. So, the inference here is, heteroscedasticity exists.  
...
Initially we consider the interaction of `age` with the other numerical features, i.e. `avg_glucose_level`, `hypertension` and `heart_disease`: we realize that only `age*heart_disease` is relevant.  
AIC = 1384
```{r}
...
summary(mod1)
```
We go on considering all the relation of `avg_glucose_level` with the remaining predictors and we find out that `avg_glucose_level*hypertension` is the best of the possible interaction even if it can not improve the previous model.    
AIC = 1385.9
```{r, results='hide'}
mod2 <- glm(stroke~age + avg_glucose_level + heart_disease+hypertension +
              avg_glucose_level*hypertension, family=binomial)
```
Ultimately, we have the interaction `heart_disease*hypertension` which seems a good version.  
AIC = 1384.5
```{r}
...
summary(mod3)
```
After that, we pick up `mod.red` without `heart_disease` which represents the explanatory feature with higher p-value and we continue to work in the same way have just seen. We repeat the scheme with `age`, without earning progresses.  
AIC = 1386.2
```{r, results='hide'}
