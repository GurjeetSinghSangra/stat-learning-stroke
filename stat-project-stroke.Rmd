...
If we liken the results, we can affirm that the reduced model seems to make a more accurate prediction and fits better data than the full one. Indeed, its AIC is the lower of the two.  
...
get.roc.recall.values <- function(pred_models, true_value, yes_plot=TRUE) {
  ...
  for (pred in pred_models) {
    ...
    pr_cutoffs <- pr_cutoffs[pr_cutoffs$recall>0.9 & pr_cutoffs$recall<=0.96, ]
...
table(training.set$stroke, qda.fit.train.pred.class, dnn=c('ground thruth','QDA 
            predicted'))
```
From the above result we can see a clear example of impact of the designed thresholds in the results of the models. We can see from the first two confusion matrices the difference between applying the ROC curve and the Precision-Recall threshold.
The former tries to minimize the false positive errors by considering just the positive results, instead the latter one, as we see from the output matrix, takes in consideration also the false negative cases. Therefore it results to minimize more the false negative rate, by allowing more errors for false positive values.  
As mentioned before in clinical cases an error committed to diagnose a disease can be very dangerous, for example when instead of predicting a stroke we classify it as a harmless case.  
For this reason, from now onwards, we will consider only the Recall-Precision values since we want to minimizes those cases (FN) and overcome the unbalance issue.
...
```
**commento al threshold, hyper-parameter**: provando più volte con la run abbiamo visto che mod1 minimizza meglio i FN
...
\textcolor{red}{We can also see that some models resulted to make more errors than expected highlighting in this way some overfitting issue.}  
In particular, we can see from the above results that the outperforming model is _____.


# 5 Conclusions and Further Analysis

non riusciamo a dire quale modello sia effettivamente il migliore, si suggerisce di usare altri modelli più complessi. Abbiamo evidenzianto che i predictors con maggior rilievo sono age e ?, nonostante questo è da valutare l'aggiunta di altri parametri.

Modello non linearly separable --> see scatter

strong relation --> beta parameters

con i dati e le analisi fatte non abbiamo racolto abbastanza informazioni sicure per poter prevenire l'ictus --> migliorare
