...
From Figure \ref{bp} we can see that in the first two boxplots (starting from the left) there are lots of outliers, whose range can also be seen from the summary by looking at the difference between the third quantile and the maximum value in the `avg_glucose_level` and `bmi` variables.  
...
Compared with the $BIC$ method, the $AIC$ statistic penalizes complex model less, meaning that it may put more emphasis on model performance on the training dataset, and, in turn, it may select more complex models. Its description is given by the following formula 
...
summary(mod.full)
```
Here we see that `age`, `avg_glucose_level` and `hypertension` are the variables most related to `stroke`, with a p-value smaller than $0.005$, but also `heart_disease` contributes a bit to the model with a p-value of $0.092381$. As expected, the collinearity factors, i.e. `work_type`, `Residence_type` and `ever_married`, do not contribute on the model.  
...
where $\beta_0, \dots, \beta_4$ can be read in the `summary` of `mod.red` with the respectively variables $X_1, \dots, X_4$ which represent `age`, `avg_glucose_level`, `heart_disease` and `hypertension`.  
...
Moreover, in the table below, there are outliers (with no connection with our regression) that have high variance, and they represent some rare cases of disease development; as we saw previously there was a case of a 14 years old girl that had stroke.
```{r, echo=FALSE}
...
summary(mod1)
```
Furthermore we can see that the interaction variable is statistically significant for a alpha value set to 0.1, hence we can say that the two slope for the two different groups, affected by the heart disease and not, are significantly different to each other. Last but not least we also see that by considering the interaction we have also a high increment in the beta value of the heart disease.
...
As we did during the training phase, the evaluation is made by looking at the false negative (FN) and false positive (FP) rates. The ratio of the misclassified non-stroke person is almost zero in all models, so we do almost not make mistake when saying to a person that they will not get a stroke disease.  
