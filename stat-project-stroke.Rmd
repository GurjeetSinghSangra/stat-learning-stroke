...
We define the final reduced model:
...
Looking at the leverage plot, we see the presence of some sample with high leverage values (bottom right), which could influence the prediction of the model (even if the R-plot does not show us its index).
...
If we compare the results, we can affirm that the reduced model seems to make a more accurate prediction and fits better data than the full one. Indeed, its AIC is the lowest.   
...
```
As expected, the anova test rejects that the complex model is more significant than the reduced one, since the p-value is not less than 5%. Therefore the full model does not help with the prediction.
...
```
However, we do not achieve anything interesting with polynomial model. There were no improvements in the results, seeing the value of the AIC which is very high.
