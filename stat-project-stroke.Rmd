...
In addition, we can see strong collinearity among the variables `age`, `work_type`, and `ever_married`, this mean that they are closely related to one another. For this reason we get uncertainty in the coefficient estimates and so we do not consider them in the fitting of the models.  
...
where $n$ is the number of samples, $d$ is the number of predictors, $\hat \sigma^2$ is an estimate of the variance of the error associated with each response measurement and $RSS$ is the residual sum of square.  
We decided to exclude the Mallow's Cp and the R^2 adjusted techniques because they work efficiently in the case of linear regression but not so good with the generalized linear models (GLM).
...
The parameters left to remove are `bmi` and `smoking_status` that continue to have a high p-value.
...
To have a confirmation of this concept, we use 'anova', a function which compares them and returns the best. The null hypothesis is that the two models fit the data equally well, and the alternative hypothesis is that the reduced model is superior.  
```{r}
