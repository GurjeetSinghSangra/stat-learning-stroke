...
###manca questo che voglio farlo appena mi funziona latex: **exp(beta) and odds for the prediction of success**
...
Looking at the leverage plot, we see the typical view where there is no influential case or no case.  
...
```
As expected, the anova test rejects that the complex model is more significant than the reduced one, since the p-value is not less than 5%. Hence the full model does not help with our prediction.
