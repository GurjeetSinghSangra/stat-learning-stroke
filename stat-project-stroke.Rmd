...
We decided to exclude the Mallow's Cp and the R^2 adjusted techniques because they work efficiently in the case of linear regression and not so good in the case of generalized linear models.
...
Also in this case the relevant variables are the same of the ones found in the full model.  
...
The variables left to eliminate are `bmi`, `smoking_status` and `working_status`.  
...
This means that our probability to get a stroke become:
...
Let's try to check other convenient models using a mixed approach. We use the reduced model `mod.red` as landmark and we proceed with interactions between the explanatory variables with the aim to understand which variables are relevant on our research and which improve the performance of the model. We remember that we move on with our selection choosing significance levels equal to $0.1$ and searching for an AIC value less and less.  
