...
Let's use the residual plots to get more information about this model (we use `type="deviance"` because of the binary response):
```{r}
par(mfrow=c(2,2))
plot(mod.full)
par(mfrow=c(1,1))
...
The variables left to eliminate are `bmi` and `smoking_status`, and we end up with the final reduced model:
...
If we compare the results, we can affirm that the reduced model seems to make a more accurate prediction and fits better the data than the full one. Indeed, the AIC is a lower number than the one of the full model. 
...
```
As expected, the anova test rejects that the complex model is more significant than the reduced one, since the p-value is not less than 5%. Hence the full model does not help with the prediction.
